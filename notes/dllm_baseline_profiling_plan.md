...
Combine D1's `comm_per_step` with `S_k` (from §3 counter) and output length. Compute `comm_per_output_token = (S_k × comm_per_step) / tokens_committed`. **Insight:** sizes the *structural* distributed penalty of diffusion decoding — how many extra collective rounds we pay per delivered token vs an AR model of the same size. This is the headline motivation number for any "fewer steps" (Design 1) or overlap (I3) work, expressed in comm terms rather than FLOPs.
